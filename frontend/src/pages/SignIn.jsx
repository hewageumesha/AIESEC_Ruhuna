...
export default function SignIn() {
  ...
  const validateForm = () => {
    let isValid = true;
    const newErrors = { aiesecEmail: '', password: '' };

    if (!formData.aiesecEmail) {
      newErrors.aiesecEmail = 'AIESEC Email is required';
      isValid = false;
    } else if (
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.aiesecEmail) ||
      !formData.aiesecEmail.endsWith('@aiesec.net')
    ) {
      newErrors.aiesecEmail = 'Please enter a valid AIESEC email address';
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://aiesecruhuna-production.up.railway.app/api/auth/signin`,
        {
          method: 'POST',
