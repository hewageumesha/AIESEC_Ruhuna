        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="bg-white divide-y divide-gray-200 dark:bg-[rgb(26,35,58)] dark:divide-gray-700">
              {departments.map((dep) => (
                <tr key={dep.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300">{dep.id}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300">{dep.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex space-x-2">
                      <Button color="warning" onClick={() => handleEdit(dep)}>Edit</Button>
                      <Button color="failure" onClick={() => openDeleteModal(dep.id)}>Delete</Button>
                    </div>
                  </td>
                </tr>
              ))}
