...
// ✅ FIXED: Updated to properly access user phone data
getMemberRegistrations: async (eventId, page = 0, size = 10) => {
  ...
  if (data.content) {
    data.content = data.content.map((registration, index) => {
      console.log(`Processing registration ${index}:`, registration); // Debug logging
      ...
      const transformedRegistration = {
        ...
        // ✅ CRITICAL FIX: Use phoneNumber field since that's what exists in User entity
       contact: (() => {
  if (!registration.user) return 'No contact';

  const phone = registration.user.phone || registration.user.phoneNumber;
  ...
  return 'No contact';
      ...
      console.log(`Transformed registration ${index}:`, transformedRegistration); // Debug logging
  ...
  getGuestRegistrations: async (eventId, page = 0, size = 10) => {
    ...
    if (data.content) {
      data.content = data.content.map(registration => ({
        ...
        // Guest phone from registration (this should already be correct)
