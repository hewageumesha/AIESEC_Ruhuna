...
export default function DashComments() {
      ...
      const membersWithCommentFlag = await Promise.all(
        membersRes.data.map(async member => {
          try {
            ...
            const commentsRes = await axios.get(`http://localhost:8080/api/comments/member/${member.id}/${currentUser.role}`, {
              headers: { Authorization: `Bearer ${currentUser.token}` }
            });
  ...
  const fetchComments = async (memberId) => {
    try {
      setIsLoading(true);
      const res = await axios.get(`http://localhost:8080/api/comments/member/${memberId}/${currentUser.role}`, {
        headers: { Authorization: `Bearer ${currentUser.token}` }
      });
