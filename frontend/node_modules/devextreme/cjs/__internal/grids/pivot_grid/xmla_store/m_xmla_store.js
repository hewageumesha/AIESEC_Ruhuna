...
const XmlaStore = exports.XmlaStore = _class.default.inherit(function() {
    ...
    function generateMdxCore(axisStrings, withArray, columns, rows, filters, slice, cubeName) {
        ...
        if (axisStrings.length) {
            ...
            mdxString = withString + (0, _string.format)("SELECT {2} FROM {0} {1} CELL PROPERTIES VALUE, FORMAT_STRING, LANGUAGE, BACK_COLOR, FORE_COLOR, FONT_FLAGS", function(columnsFilter, rowsFilter, filter, cubeName) {
                let from = `[${cubeName}]`;
                ...
                return from
            }(generateAxisFieldsFilter(columns), generateAxisFieldsFilter(rows), generateAxisFieldsFilter(filters || []), cubeName), slice.length ? (0, _string.format)("WHERE ({0})", slice.join(",")) : "", select)
        }
        return mdxString
    }
