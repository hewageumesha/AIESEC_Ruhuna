    ...
    dockHeader(eventData) {
        ...
        if (targetDraggingPanel) {
            ...
            const pointsByColumns = "columnChooser" === targetLocation ? [] : pointsByTarget[targetLocation] || controller._generatePointsByColumns((0, _extend.extend)({}, dragOptions, {
                ...
                startColumnIndex: "headers" === targetLocation ? (0, _renderer.default)(columnElements[0]).index() : 0
            }));
...
class ColumnsResizerViewController extends _m_modules.default.ViewController {
    ...
    _setupResizingInfo(posX) {
        ...
        this._resizingInfo = {
            ...
            currentColumnWidth: null !== $currentHeader && void 0 !== $currentHeader && $currentHeader.length ? (0, _position.getBoundingRect)($currentHeader[0]).width : 0,
            nextColumnIndex: nextColumnIndex,
            nextColumnWidth: null !== $nextHeader && void 0 !== $nextHeader && $nextHeader.length ? (0, _position.getBoundingRect)($nextHeader[0]).width : 0,
    ...
    _generateColumnsTopYIndex() {
        ...
        for (let rowIndex = 0; rowIndex < rowCount - 1; rowIndex++) {
            ...
            if (cells && cells.length > 0) {
                _m_utils.default.getPointsByColumns(cells, pointCreated, false, 0, needToCheckPrevPoint)
            }
        }
        return topYMap
    }
    _generatePointsByColumns() {
        ...
        if (cells && cells.length > 0) {
            that._pointsByColumns = _m_utils.default.getPointsByColumns(cells, (point => that._pointCreated((point => {
                const x = Math.ceil(point.x);
                if (topYMap[x]) {
                    point.y = topYMap[x]
                }
...
class DraggingHeaderViewController extends _m_modules.default.ViewController {
    ...
    drag(parameters) {
        ...
        if (sourceColumnElement) {
            sourceColumnElement.addClass(this.addWidgetPrefix(_const.CLASSES.draggableColumn));
    ...
    drop(parameters) {
        ...
        if (sourceColumnElement) {
            sourceColumnElement.removeClass(this.addWidgetPrefix(_const.CLASSES.draggableColumn));
