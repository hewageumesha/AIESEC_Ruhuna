...
const isAllDayAppointment = appointment => {
    ...
    return Boolean(null === (_appointment$settings = appointment.settings[0]) || void 0 === _appointment$settings ? void 0 : _appointment$settings.allDay)
};
class SchedulerAppointments extends _uiCollection_widget.default {
    ...
    _getNavigatableItemByIndex(sortedIndex) {
        ...
        return appointments.filter(((_, $item) => (0, _element_data.data)($item, _constants.APPOINTMENT_SETTINGS_KEY).sortedIndex === sortedIndex)).eq(0)
    }
    ...
    _getGroupTexts(groupIndex, loadedResources) {
        if (!(null !== loadedResources && void 0 !== loadedResources && loadedResources.length)) {
            return []
        }
        const idPath = (0, _m_utils.getPathToLeaf)(groupIndex, loadedResources);
        const textPath = idPath.map(((id, index) => loadedResources[index].items.find((item => item.id === id)).text));
        return textPath
    }
    _renderAppointment(element, settings) {
        ...
        if (settings.virtual) {
            ...
            this._createComponent(element, this.isAgendaView ? _agenda_appointment.AgendaAppointment : _m_appointment.Appointment, _extends({}, config, {
    ...
    updateResizedAppointment($element, dateRange, dataAccessors, timeZoneCalculator) {
        const sourceAppointment = this._getItemData($element);
        const gridAdapter = (0, _m_appointment_adapter.createAppointmentAdapter)(sourceAppointment, dataAccessors, timeZoneCalculator).clone();
        ...
        convertedBackAdapter.calculateDates("fromGrid").calculateDates("toGrid");
    ...
    _getEndResizeAppointmentStartDate(e, rawAppointment, appointmentInfo) {
        const timeZoneCalculator = this.option("timeZoneCalculator");
        const appointmentAdapter = (0, _m_appointment_adapter.createAppointmentAdapter)(rawAppointment, this.dataAccessors, timeZoneCalculator);
        let {
            startDate: startDate
        } = appointmentInfo;
    ...
    _sortAppointmentsByStartDate(appointments) {
        return (0, _m_utils2.sortAppointmentsByStartDate)(appointments, this.dataAccessors)
    }
    ...
    _processLongAppointment(appointment, result) {
        ...
        if (partCount > 1) {
            ...
            for (let i = 1; i < partCount; i++) {
                ...
                startDate = timeZoneCalculator.createDate(startDate.getTime(), {
                    path: "toGrid"
                });
    ...
    splitAppointmentByDay(appointment) {
        ...
        const endDayHour = this.invoke("getEndDayHour");
        const timeZoneCalculator = this.option("timeZoneCalculator");
        const adapter = (0, _m_appointment_adapter.createAppointmentAdapter)(appointment, this.dataAccessors, timeZoneCalculator);
