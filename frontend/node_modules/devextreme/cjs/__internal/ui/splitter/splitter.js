...
class Splitter extends _m_collection_widget.default {
    ...
    _getItemInstance($item) {
        return _splitter_item.default.getInstance($item)
    }
    ...
    _updateResizeHandleCollapsibleState(resizeHandle) {
    ...
    _getResizeHandleConfig(paneId) {
        ...
        return {
            ...
            onResize: e => {
                ...
                const newLayout = (0, _layout.getNextLayout)(this._currentLayout ?? [], (0, _layout.calculateDelta)(event.offset, currentOrientation, rtlEnabled, this._currentOnePxRatio), this._activeResizeHandleIndex, this._itemRestrictions);
    ...
    _itemOptionChanged(item, property, value, prevValue) {
        switch (property) {
            ...
            case "visible":
                ...
                this._itemVisibleOptionChanged(item, value, prevValue);
                break;
            case "template":
                super._itemOptionChanged(item, property, value, prevValue);
                this._applyStylesFromLayout(this.getLayout());
    ...
    _itemVisibleOptionChanged(item) {
        let value = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : true;
        let prevValue = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : true;
        ...
        const {
            direction: direction,
            paneIndex: paneIndex
        } = this._getDistributionData(item, value);
        this._updateResizeHandles();
        if (paneIndex && paneIndex < 0) {
            return
        }
        this._updateItemsRestrictions();
        const collapsedDelta = this._getCollapseDelta(item, !value, this._panesCacheSizeVisible, direction);
        this._itemRestrictions.forEach((pane => {
            pane.maxSize = void 0;
            pane.resizable = void 0
        }));
        this._layout = (0, _layout.getNextLayout)(this.getLayout(), collapsedDelta, paneIndex, this._itemRestrictions);
        this._applyStylesFromLayout(this.getLayout());
        this._updateItemSizes()
    }
    _getDistributionData(item, value) {
        const itemIndex = this._getIndexByItem(item);
        const $item = (0, _renderer.default)(this._itemElements()[itemIndex]);
        const {
            items: items = []
        } = this.option();
        const isLastVisible = itemIndex >= (0, _layout.findLastIndexOfVisibleItem)(items);
        const direction = isLastVisible === (false === value) ? _types.CollapseExpandDirection.Next : _types.CollapseExpandDirection.Previous;
        const paneIndex = this._getPaneIndexByElement(isLastVisible ? this._getResizeHandleLeftItem($item) : $item);
        return {
            direction: direction,
            paneIndex: paneIndex
        }
    ...
    _updateResizeHandles() {
        this._iterateItems(((_, itemElement) => {
            const item = this._getItemInstance((0, _renderer.default)(itemElement));
    ...
    _getPaneIndexByElement($element) {
    ...
    _getCollapseDelta(item, newCollapsedState, panesCacheSize, direction) {
        const itemIndex = this._getIndexByItem(item);
        const {
        ...
        if (newCollapsedState) {
            ...
            if (currentPaneSize > targetPaneSize) {
                panesCacheSize[itemIndex] = {
                    ...
                    direction: direction === _types.CollapseExpandDirection.Next ? _types.CollapseExpandDirection.Previous : _types.CollapseExpandDirection.Next
                }
            }
            const delta = direction === _types.CollapseExpandDirection.Previous ? targetPaneSize - currentPaneSize : currentPaneSize - targetPaneSize;
            return delta
        }
        const paneCache = panesCacheSize[itemIndex];
        panesCacheSize[itemIndex] = void 0;
        let targetPaneSize = 0;
        if (paneCache && paneCache.direction === direction) {
            targetPaneSize = paneCache.size - collapsedSize
        } else {
            targetPaneSize = direction === _types.CollapseExpandDirection.Previous ? this._calculateExpandToLeftSize(itemIndex - 1) : this._calculateExpandToRightSize(itemIndex + 1)
        }
        ...
        const deltaSign = direction === _types.CollapseExpandDirection.Previous ? -1 : 1;
