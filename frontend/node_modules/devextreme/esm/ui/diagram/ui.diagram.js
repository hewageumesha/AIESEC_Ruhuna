...
const FULLSCREEN_CHANGE_EVENT_NAME = addNamespace("fullscreenchange", "dxDiagramEvent");
const IE_FULLSCREEN_CHANGE_EVENT_NAME = addNamespace("msfullscreenchange", "dxDiagramEvent");
const WEBKIT_FULLSCREEN_CHANGE_EVENT_NAME = addNamespace("webkitfullscreenchange", "dxDiagramEvent");
const MOZ_FULLSCREEN_CHANGE_EVENT_NAME = addNamespace("mozfullscreenchange", "dxDiagramEvent");
class Diagram extends Widget {
    ...
    _initMarkup() {
        ...
        const $contentWrapper = $("<div>").addClass("dx-diagram-content-wrapper").appendTo(this.$element());
        ...
        this._$content = $("<div>").addClass("dx-diagram-content").appendTo($contentWrapper);
        ...
        if (!isServerSide) {
            const $scrollViewWrapper = $("<div>").addClass("dx-diagram-scroll-view").appendTo(this._$content);
    ...
    isMobileScreenSize() {
        if (void 0 === this._isMobileScreenSize) {
            this._isMobileScreenSize = hasWindow() && getOuterWidth(this.$element()) < 576
        }
    ...
    _renderMainToolbar() {
        const $toolbarWrapper = $("<div>").addClass("dx-diagram-toolbar-wrapper").appendTo(this.$element());
    ...
    _renderHistoryToolbar($parent) {
        const $container = $("<div>").addClass("dx-diagram-floating-toolbar-container").appendTo($parent);
    ...
    _getToolboxBounds($parent, isServerSide) {
        const result = {
            offsetX: 12,
            offsetY: 12,
            height: !isServerSide ? getHeight($parent) - 24 : 0
        };
        if (this._historyToolbar && !isServerSide) {
            result.offsetY += getOuterHeight(this._historyToolbar.$element()) + 12;
            result.height -= getOuterHeight(this._historyToolbar.$element()) + 12
        }
    ...
    _renderPropertiesToolbar($parent) {
        const isServerSide = !hasWindow();
        const $container = $("<div>").addClass("dx-diagram-floating-toolbar-container").addClass("dx-diagram-properties-panel-toolbar-container").appendTo($parent);
    ...
    _renderPropertiesPanel($parent) {
        ...
        const offsetY = 24 + (!isServerSide ? getOuterHeight(this._propertiesToolbar.$element()) : 0);
    ...
    _showLoadingIndicator() {
        this._loadingIndicator = $("<div>").addClass("dx-diagram-loading-indicator");
