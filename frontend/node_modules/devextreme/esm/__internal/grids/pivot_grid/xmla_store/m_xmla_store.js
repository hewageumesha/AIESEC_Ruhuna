...
import {
    getLanguageId
} from "../../../../common/core/localization/language_codes";
import {
    errors
...
const XmlaStore = Class.inherit(function() {
    ...
    function generateAxisMdx(options, axisName, cells, withArray, parseOptions) {
        ...
        let expandLevel = 0;
        const result = [];
        const cellsString = stringFormat("{{0}}", cells.join(","));
    ...
    function generateMdxCore(axisStrings, withArray, columns, rows, filters, slice, cubeName) {
        ...
        if (axisStrings.length) {
            ...
            mdxString = withString + stringFormat("SELECT {2} FROM {0} {1} CELL PROPERTIES VALUE, FORMAT_STRING, LANGUAGE, BACK_COLOR, FORE_COLOR, FONT_FLAGS", function(columnsFilter, rowsFilter, filter, cubeName) {
                let from = `[${cubeName}]`;
                ...
                return from
            }(generateAxisFieldsFilter(columns), generateAxisFieldsFilter(rows), generateAxisFieldsFilter(filters || []), cubeName), slice.length ? stringFormat("WHERE ({0})", slice.join(",")) : "", select)
        }
        return mdxString
    }
    ...
    function preparePathValue(pathValue, dataField) {
        if (pathValue) {
            const shouldSkipWrappingPathValue = isString(pathValue) && (pathValue.includes("&") || pathValue.startsWith(`${dataField}.`));
