...
import {
    getScrollTopMax
} from "../../ui/scroll_view/utils/get_scroll_top_max";
import {
    isElementVisible
} from "../splitter/utils/layout";
...
export const MESSAGEGROUP_TIMEOUT = 3e5;
class MessageList extends Widget {
    ...
    _initMarkup() {
        $(this.element()).addClass("dx-chat-messagelist");
        super._initMarkup();
    ...
    _renderEmptyViewContent() {
        const $emptyView = $("<div>").addClass("dx-chat-messagelist-empty-view").attr("id", `dx-${new Guid}`);
        $("<div>").appendTo($emptyView).addClass("dx-chat-messagelist-empty-image");
        const messageText = messageLocalization.format("dxChat-emptyListMessage");
        $("<div>").appendTo($emptyView).addClass("dx-chat-messagelist-empty-message").text(messageText);
        const promptText = messageLocalization.format("dxChat-emptyListPrompt");
        $("<div>").appendTo($emptyView).addClass("dx-chat-messagelist-empty-prompt").text(promptText);
    ...
    _createDayHeader(timestamp) {
        ...
        $("<div>").addClass("dx-chat-messagelist-day-header").text(headerDate).appendTo(this._$content)
    }
    ...
    _renderMessageListContent() {
        this._$content = $("<div>").addClass("dx-chat-messagelist-content").appendTo(this._$scrollViewContent())
    }
    _toggleEmptyView() {
    ...
    _scrollableContainer() {
        return $(this._scrollView.element()).find(".dx-scrollable-container").get(0)
    }
    ...
    _isTimeoutExceeded(lastMessage, newMessage) {
        ...
        const result = newMessageTimestampInMs - lastMessageTimestampInMs > 3e5;
        return result
    }
    _updateAria() {
        const aria = {
