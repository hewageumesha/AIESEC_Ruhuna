...
//    }
    ...
    @PutMapping("/task/{taskId}/updateStatusWithProof")
    public ResponseEntity<?> updateTaskStatusWithProof(
            @PathVariable Integer taskId,
            @RequestParam String status
    ) {
        try {
            if ("completed".equalsIgnoreCase(status)) {
                boolean proofExists = taskProofRepo.existsByTask_TaskId(taskId);
                if (!proofExists) {
                    return ResponseEntity.badRequest()
                            .body(Map.of("error", "Cannot mark as completed without uploading proof."));
                }
            }

            // Call your existing method (so no duplication of update logic)
            taskService.updateTaskStatus(taskId, status);
