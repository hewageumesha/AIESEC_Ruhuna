...
const TaskList: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-100 shadow-lg rounded-lg">
      {userDetails && <h2 className="text-2xl font-bold text-gray-700 mb-4">Task List for {userDetails.username}</h2>}
      <h2 className="text-xl font-semibold text-gray-600 mb-3">All Your Tasks</h2>
        ...
        <tbody>
          {tasks.map((task) => (
            <tr key={task.taskId} className="border-b hover:bg-gray-200">
              ...
              <td className={`py-2 px-4 ${task.workOfStatus === 'Complete' ? 'text-green-600' : 'text-red-600'}`}>{task.workOfStatus}</td>
              <td className="py-2 px-4 flex gap-2 justify-center">
                ...
                {task.workOfStatus !== "Complete" && (
                  <button className="text-green-500 hover:underline" onClick={() => handleMarkAsComplete(task.taskId)}>Mark as Complete</button>
                )}
              </td>
            </tr>
          ))}
