...
const BirthdayForm = () => {
  const [formData, setFormData] = useState({ name: "", date: "", role: "", photo: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: "", date: "", role: "", photo: "" });
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md">
      {["name", "date", "role", "photo"].map((field) => (
        <div className="mb-4" key={field}>
          <label htmlFor={field} className="block text-gray-700 font-medium mb-2 capitalize">
            {field === "photo" ? "Photo URL" : field === "role" ? "Role" : field === "date" ? "Date of Birth" : "Name"}
          </label>
          <input
